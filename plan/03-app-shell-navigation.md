...
## 작업 항목
- [ ] `MainActivity` + `NavHostFragment`. `@AndroidEntryPoint`.
- [ ] `nav_graph.xml`: 4 destination + action. 시작 destination = Home.
- [ ] 4 Fragment 골격(ViewBinding, 빈 레이아웃 + 임시 라벨). Hilt `@AndroidEntryPoint`.
- [ ] 인자 계약 정의: Select→Analyze(선택 사진 식별자 목록·타임존), Analyze→Map(tripId), Home→Map(tripId). **대용량 사진 목록은 Bundle에 직접 싣지 말고** 공유 ViewModel/리포지토리 핸들로 전달(04 연계).
- [ ] 뒤로가기 정책: Map/Analyze에서 뒤로 → Home로 백스택 pop, 진행 중 타이머·분석 취소 훅(11·10 연계) 지점 마련.
- [ ] 화면별 공유 상태 보관용 ViewModel 스캐폴딩(트립 컨텍스트).
- [ ] 시스템 인셋/상태바 처리(프로토타입의 커스텀 상태바 UI는 이식하지 않고 실제 시스템 상태바 사용).
...
## 완료 조건 (DoD)
- [ ] Home→Select→Analyze→Map→Home 왕복이 stub 상태로 실제 전환됨.
- [ ] 여행 카드 탭 → Map 직접 진입 경로 동작(더미 tripId).
- [ ] 뒤로가기 시 백스택·리소스 정리 훅이 호출됨(로그로 확인).
- [ ] 회전/프로세스 재생성 시 현재 화면 복원(기본 수준).
